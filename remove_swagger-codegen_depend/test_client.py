...
def test_index_list_with_params(client, user):
    data1 = get_doc()
    data1["urls"] = ["s3://endpointurl/bucket_2/key_2", "s3://anotherurl/bucket_2/key_2"]
    data1["urls_metadata"] = {
...
def test_list_entries_with_uploader(client, user):
    ...
    assert {rec_2["did"], rec_3["did"]} == {data_list["records"][0]["did"], data_list["records"][1]["did"]}
...
def test_get_empty_acl_authz_record_after_fill_size_n_hash(client, user):
    ...
    client.put("/index/blank/"+rec_3["did"]+"?rev="+rec_3["rev"], json=updated, headers=user)
...
def test_urls_metadata(client, user):
    ...
    client.put("/index/"+rec_2["did"]+"?rev="+rec_2["rev"], json=updated, headers=user)
...
def test_delete_and_recreate(client, user):
    ...
    # delete the old doc
    client.delete("/index/"+old_record["did"]+"?rev="+old_record["rev"], json=old_data, headers=user)
...
def test_index_update(client, user):
    ...
    dataNew["acl"] = ["a", "b"]
    dataNew["authz"] = ["x", "y"]
    res_2 = client.put("/index/"+rec["did"]+"?rev="+rec["rev"], json=dataNew, headers=user)
    rec_2 = res_2.json
...
def test_update_uploader_field(client, user):
    ...
    client.put("/index/"+rec["did"]+"?rev="+rec["rev"], json=updated, headers=user)
...
def test_index_delete(client, user):
    ...
    client.delete("/index/"+rec["did"]+"?rev="+rec["rev"], json=data, headers=user)
...
def test_alias_update(client, user):
    ...
    res_2 = client.put("/alias/"+ark+"?rev="+rec_1["rev"], json=dataNew, headers=user)
    rec_2 = res_2.json
...
def test_dos_get(client, user):
    ...
    assert rec_2["data_object"]["checksums"][0]["checksum"] == "8b9942cf415384b27cadf1f4d2d682e5"
    ...
    assert rec_2["data_object"]["urls"][0]["system_metadata"]["project_id"] == "bpa-UChicago"
...
def test_dos_list(client, user):
    ...
    assert rec_2["data_objects"][0]["checksums"][0]["checksum"] == "8b9942cf415384b27cadf1f4d2d682e5"
    ...
    assert rec_2["data_objects"][0]["urls"][0]["system_metadata"]["project_id"] == "bpa-UChicago"
...
def test_update_without_changing_fields(client, user):
    ...
    client.put("/index/"+first_doc["did"]+"?rev="+first_doc["rev"], json=updated, headers=user)
    ...
    client.put("/index/"+second_doc["did"]+"?rev="+second_doc["rev"], json=updated, headers=user)
...
def test_bulk_get_documents(client, user):
    ...
    dids = [client.post("/index/", json=get_doc(has_baseid=True), headers=user).json["did"] for _ in range(20)]
