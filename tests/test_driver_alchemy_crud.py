...
def test_driver_add_with_urls():
    ...
    with sqlite3.connect('index.sq3') as conn:
        ...
        for url in (conn.execute('''
            SELECT index_record_url.url,index_record_api.api FROM index_record_url, index_record_api WHERE index_record_url.url == index_record_api.url
        ''')):
            if url[0] not in new_urls:
                new_urls[url[0]] = [url[1]]
            else:
                new_urls[url[0]].append(url[1])
...
def test_driver_update_record():
    ...
    with sqlite3.connect('index.sq3') as conn:
        ...
        update_size = 256
        update_urls = {'a': ['api1'], 'b': ['api2'], 'c': ['api3']}
        update_hashes = {
            'a': '1',
            'b': '2',
            'c': '3',
...
@util.removes('index.sq3')
def test_driver_update_fails_with_no_recods():
    '''
