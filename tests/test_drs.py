...
def generate_presigned_url_response(did, protocol=""):
    full_url = settings['config']['PRESIGNED_URL_ENDPT']+'data/download/' + did + '?protocol=' + protocol
...
def test_drs_get_with_presigned_url(client, user):
    ...
    res_2 = client.get("/ga4gh/drs/v1/objects/" + rec_1["did"], headers = {"AUTHORIZATION": "12345"})
    assert res_2.status_code == 200
    rec_2 = res_2.json
    assert rec_2["drs_object"]["id"] == rec_1["did"] 
    assert rec_2["drs_object"]["size"] == 123
...
def test_get_presigned_url(client, user):
    ...
    for access_id in access_id_list:
        ...
        res_2 = client.get("/ga4gh/drs/v1/objects/" + rec_1["did"] + "/access/" + access_id)
        assert res_2.status_code == 200
