...
def test_drs_get(app_client, user, combined_default_and_single_table_settings):
    ...
    assert "authorizations" in rec_2["access_methods"][0]
...
def verify_timestamps(expected_doc, did, client, has_updated_date=True):
    ...
    drs_record = drs_resp.json()
    record_resp = client.get(f"/index/{did}")
    assert record_resp.status_code == 200
    record = record_resp.json()
    assert expected_doc["content_created_date"] == drs_record["created_time"]
    assert expected_doc["content_created_date"] == record["content_created_date"]
    if has_updated_date:
        assert expected_doc["content_updated_date"] == drs_record["updated_time"]
        assert expected_doc["content_updated_date"] == record["content_updated_date"]
    else:
        assert expected_doc["content_created_date"] == drs_record["updated_time"]
        assert expected_doc["content_created_date"] == record["content_updated_date"]
    assert drs_record["index_created_time"] == record["created_date"]
...
def test_timestamps_none(app_client, user, combined_default_and_single_table_settings):
    ...
    assert record_resp.status_code == 200
    record = record_resp.json()
    assert drs_record.get("created_time") is None
    assert drs_record.get("updated_time") is None
    assert record.get("content_created_date") is None
    assert record.get("content_updated_date") is None
    assert drs_record["index_created_time"] == record["created_date"]
    assert drs_record["index_updated_time"] == record["updated_date"]
...
def test_service_info_custom_bulk_limit(
    ...
    try:
        ...
        drs_router.max_bulk_request_length = original
...
def test_get_cloud_provider_string_mapping():
    original = drs_router.cloud_provider_map
    try:
        drs_router.cloud_provider_map = {
            "s3": "aws",
            "gs": "gcp",
            "az": "azure",
        }
        ...
        drs_router.cloud_provider_map = original
...
def test_get_cloud_provider_unknown_protocol():
    ...
    try:
        ...
        assert get_cloud_provider("ftp://example.com/path/to/file") is None
    finally:
        drs_router.cloud_provider_map = original
...
def test_get_cloud_provider_empty_string_mapping_returns_unknown():
    ...
    try:
        ...
        assert get_cloud_provider("s3://my-bucket/path/to/file") is None
    finally:
        drs_router.cloud_provider_map = original
...
def test_get_cloud_provider_https_prefix_matching():
    ...
    try:
        ...
        assert get_cloud_provider("https://m3.aicommons.com/other/file") is None
...
def test_single_path_not_found(
    ...
    expected_metadata_details = {
        ...
        "passport_auth_issuers": ["test_passport_default"],
    }
    # Test GET
...
def test_bulk_auth_options_malformed_error(
    ...
    assert res_1.json()["error"] == "Request is malformed. Missing bulk object ids."


def test_preferred_type(app_client, user, combined_default_and_single_table_settings):
    """
    ...
    """
    _, client = app_client
    original = drs_router.default_preferred_type
    try:
        drs_router.default_preferred_type = "PassportAuth"
        data = get_doc(
        ...
        drs_router.default_preferred_type = original
...
def test_preferred_type_authz_override(
    ...
    doc_did = client.post("/index/", json=data, headers=user)
    doc_did = doc_did.json()["did"]
    expected_supported_types = ["PassportAuth", "BearerAuth"]
    res_1 = client.options("ga4gh/drs/v1/objects/" + doc_did)
    assert res_1.status_code == 200
    res1_json = res_1.json()
