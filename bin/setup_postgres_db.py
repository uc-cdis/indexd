...
def create_tables(host, user, password, database):
    ...
    create_index_record_stm = "CREATE TABLE index_record (\
    	did VARCHAR NOT NULL, rev VARCHAR, form VARCHAR, size INTEGER, PRIMARY KEY (did) )"
    ...
    try:
        ...
        logging.warn('Unable to create table: {}'.format(msg))
    conn.close()
...
if __name__ == '__main__':
	...
	parser.add_argument("--database", type=str, action="store",
        default='test_migration_db', help="psql test database")
	...
	try_drop_test_data(args.user, args.database, root_user='postgres', host=args.host)
